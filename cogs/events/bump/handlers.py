...
class BumpHandler:
    ...
    async def check_cooldown(
        self, session, user_id: int, service: str, cooldown_hours: int
    ) -> bool:
    ...
    async def log_notification(
        self, session, user_id: int, service: str
    ) -> None:
...
class DisboardHandler(BumpHandler):
    ...
    async def handle(self, message: discord.Message) -> None:
        ...
        if hasattr(self.bot.get_cog('OnBumpEvent'), 'send_bump_marketing'):
            await self.bot.get_cog('OnBumpEvent').send_bump_marketing(
                message.channel, "disboard", user
            )

    async def send_thank_you(
        self, channel: discord.TextChannel, user: discord.Member, service: str
    ) -> None:
...
class DiscadiaHandler(BumpHandler):
    ...
    async def handle_bump(self, message: discord.Message) -> bool:
        ...
        if message.embeds:
            ...
            embed_content = (
                f"{embed.title or ''} {embed.description or ''}".lower()
            )
    ...
    async def handle(self, message: discord.Message) -> None:
        ...
        async with self.bot.get_db() as session:
            ...
            if await self.check_cooldown(
                session, user.id, "discordservers", SERVICE_COOLDOWNS["discordservers"]
            ):
            ...
            # Add bypass time
            await self.add_bypass_time(
                user, BYPASS_DURATIONS["discordservers"], "discordservers"
            )
