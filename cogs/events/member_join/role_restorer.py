...
class RoleRestorer:
            ...
            try:
                ...
                for member_role in member_roles:
                    ...
                    if role.name in mute_role_names:
                        ...
                        mute_type = next(
                            (r["description"] for r in self.bot.config.get("mute_roles", []) 
                             if r["name"] == role.name), 
                            "unknown"
                        )
                ...
                if roles_to_restore:
                    try:
                        ...
                        if special_roles_info:
                            logger.info(
                                f"Przywrócono role specjalne dla {member}: {', '.join(special_roles_info)}"
                            )
    ...
    async def _notify_restored_roles(
        ...
        try:
            ...
            if special_info:
                embed.add_field(
                    name="Informacje specjalne",
                    value="\n".join(special_info[:5]),  # Limit to 5
                    inline=False
                )
    ...
    async def restore_voice_permissions(self, member: discord.Member) -> int:
        ...
        try:
            ...
            async with self.bot.get_db() as session:
                ...
                permissions = await ChannelPermissionQueries.get_member_permissions(
                    session, member.id
                )
                ...
                for perm in permissions:
                    ...
                    try:
                        ...
                        await channel.edit(
                            ...
                            reason=f"Przywracanie uprawnień dla {member}"
                        )
                        
                        restored_count += 1
                        logger.info(
                            f"Restored voice permissions for {member} in channel {channel.name}"
                        )
                        
                    except discord.Forbidden:
                        logger.error(
                            f"No permission to restore voice permissions in {channel.name}"
                        )
                    except discord.HTTPException as e:
                        logger.error(
                            f"Failed to restore voice permissions in {channel.name}: {e}"
                        )
                ...
                if restored_count > 0:
                    ...
                    logger.info(
                        f"Total voice permissions restored: {self.voice_permissions_restored}"
                    )
