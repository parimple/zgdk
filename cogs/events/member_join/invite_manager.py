        ...
        async with self.bot.get_db() as session:
            try:
                ...
                for db_invite in all_invites:
                    ...
                    if hasattr(db_invite, 'max_age_seconds') and db_invite.max_age_seconds and db_invite.max_age_seconds > 0:
                        expiry_time = db_invite.created_at + timedelta(
                            seconds=db_invite.max_age_seconds
                        )
                    ...
                    if (hasattr(db_invite, 'max_uses') and db_invite.max_uses and db_invite.max_uses > 0 and 
                        hasattr(db_invite, 'uses') and db_invite.uses >= db_invite.max_uses):
                        should_delete = True
                        logger.info(
                            f"Invite {db_invite.code} reached max uses "
                            f"({db_invite.uses}/{db_invite.max_uses})"
                        )
