...
class WelcomeMessageSender:
    ...
    def _create_log_embed(
        ...
        embed.add_field(name="Invite Info", value=invite_info, inline=True)
        ...
        embed.add_field(
            name="Status", value="\n".join(status_info) if status_info else "No special status", inline=True
        )
    ...
    async def _send_log_message(
        self, member: discord.Member, embed: discord.Embed, inviter: Optional[discord.User]
    ) -> None:
        ...
        await self.welcome_channel.send(embed=embed, allowed_mentions=AllowedMentions.none())
        logger.info(f"Logged member join for {member}")

    async def send_error_notification(self, member: discord.Member, error_type: str, error_details: str) -> None:
        ...
        embed.add_field(name="Typ błędu", value=error_type, inline=True)

        embed.add_field(name="Szczegóły", value=error_details[:1024], inline=False)  # Limit to 1024 chars
