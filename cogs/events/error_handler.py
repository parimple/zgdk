...
class ErrorHandlerCog(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, error: Exception):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            validation_error = ValidationException(f"Brakujący argument: `{error.param.name}`")
            await self.error_handler.handle_command_error(ctx, validation_error)
            return
        ...
        if isinstance(error, commands.CommandInvokeError):
            ...
            logger.error(f"Unexpected error in command {ctx.command}: {original_error}", exc_info=original_error)

            # Send generic error message
            await ctx.send(
                "❌ Wystąpił nieoczekiwany błąd podczas wykonywania komendy. " "Administratorzy zostali powiadomieni."
            )
            return
    ...
    async def on_application_command_error(self, interaction: discord.Interaction, error: Exception):
        ...
        # Log unexpected errors
        logger.error(f"Unexpected error in slash command: {error}", exc_info=error)
