...
class RankingCommands(commands.Cog):
    ...
    async def ranking(self, ctx: commands.Context, days: int = 7, limit: int = 10):
        ...
        try:
            ...
            async with self.bot.get_db() as session:
                ...
                leaderboard = await activity_service.get_leaderboard(session, limit, days)
    ...
    async def stats(self, ctx: commands.Context, member: discord.Member = None, days: int = 7):
        ...
        try:
            async with self.bot.get_db() as session:
                ...
                stats = await activity_service.get_member_stats(session, target_member.id, days)
    ...
    @commands.hybrid_command(
        name="my_rank", aliases=["mojapozcyja", "mojrank"], description="Pokaż swoją pozycję w rankingu"
    )
    ...
    @commands.hybrid_command(name="topusers", description="Pokaż TOP użytkowników w różnych kategoriach")
    ...
    async def top(self, ctx: commands.Context, category: str = "100", days: int = 7):
        ...
        try:
            ...
            if ctx.guild.icon:
                embed.set_footer(
                    text=f"💡 Aktualizacja: co godzinę | Dane z ostatnich {days} dni", icon_url=ctx.guild.icon.url
                )
    ...
    @commands.check(
        lambda ctx: ctx.cog.permission_service and ctx.cog.permission_service.is_owner(ctx.author)
        if hasattr(ctx.cog, "permission_service")
        else False
    )
