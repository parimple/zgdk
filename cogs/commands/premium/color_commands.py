        ...
        async with self.bot.get_db() as session:
            ...
            if premium_service and ctx.guild:
                ...
                if not has_premium:
                    ...
                    if premium_roles:
                        role_mentions = [f"<@&{role['role_id']}>" for role in premium_roles if role.get('perks', {}).get('custom_role_color')]
                        if role_mentions:
    ...
    async def color(self, ctx, *, color: Optional[str] = None):
        ...
        async with self.bot.get_db() as session:
            ...
            if not has_color_permission:
                await self._send_premium_embed(
                    ctx,
                    description="Nie masz uprawnień do używania kolorów ról.",
                    color=0xFF0000
                )
                return
        ...
        if color is None:
            ...
            if color_role:
                ...
                await self._send_premium_embed(
                    ctx,
                    description="✅ Twój kolor roli został usunięty.",
                    color=0x00FF00
                )
                ...
                await self._send_premium_embed(
                    ctx,
                    description="Nie masz ustawionego koloru roli.",
                    color=0xFF0000
                )
            return
        ...
        try:
            ...
            await self._send_premium_embed(
                ctx,
                description=f"❌ Błąd podczas ustawiania koloru: {str(e)}",
                color=0xFF0000
            )
        except Exception as e:
            ...
            await self._send_premium_embed(
                ctx,
                description="❌ Wystąpił nieoczekiwany błąd podczas ustawiania koloru.",
                color=0xFF0000
            )