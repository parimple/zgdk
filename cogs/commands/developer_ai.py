...
class DeveloperAI(commands.Cog):
    ...
    async def explain_decision(
        self, ctx: commands.Context, trace_id: Optional[int] = None, module: Optional[str] = None
    ):
        """
        ...
        embed.add_field(name="Moduł", value=decision.module, inline=True)
        embed.add_field(name="Pewność", value=f"{decision.confidence:.0%}", inline=True)
        embed.add_field(name="Czas wykonania", value=f"{decision.execution_time_ms:.1f}ms", inline=True)

        # Add input/output
        embed.add_field(name="Wejście", value=f"```{decision.input_data.get('text', 'N/A')[:100]}```", inline=False)
        embed.add_field(name="Decyzja", value=f"```{str(decision.final_decision)[:100]}```", inline=False)
        ...
        if decision.features_extracted:
            ...
            embed.add_field(name="Kluczowe cechy", value="\n".join(feature_list), inline=False)
    ...
    async def show_features(self, ctx: commands.Context, module: str):
        ...
        # Send as embed
        embed = discord.Embed(
            title=f"📊 Raport Cech: {module}", description=report[:2000], color=discord.Color.green()  # Discord limit
        )
    ...
    @commands.is_owner()
    async def show_traces(self, ctx: commands.Context, limit: Optional[int] = 5, module: Optional[str] = None):
        """
    ...
    async def show_stats(self, ctx: commands.Context):
        ...
        for decision in all_decisions:
            ...
            if module not in module_stats:
                module_stats[module] = {"count": 0, "total_time": 0, "avg_confidence": 0, "success_count": 0}
        ...
        for module, stats in module_stats.items():
            ...
            embed.add_field(name=f"📦 {module}", value="\n".join(info), inline=True)
    ...
    @commands.is_owner()
    async def test_ai(self, ctx: commands.Context, module: str, *, test_input: str):
        """
        ...
        try:
            ...
            decisions = self.explainer.logger.get_recent_decisions(
                limit=1, module=f"{module}_parser" if module != "intent" else "intent_classifier"
            )

            embed = discord.Embed(
                title=f"🧪 Test AI: {module}", description=f"**Wejście**: `{test_input}`", color=discord.Color.blue()
            )

            embed.add_field(name="Wynik", value=response, inline=False)
            ...
            if decisions:
                ...
                embed.add_field(name="Wyjaśnienie", value=explanation[:1024], inline=False)
    ...
    async def debug_ai_state(self, ctx: commands.Context):
        ...
        # Check feature usage
        total_features = sum(sum(counts.values()) for counts in self.explainer.logger.feature_usage.values())
        info.append(f"**Feature extractions**: {total_features}")

        embed = discord.Embed(title="🔧 AI System Debug", description="\n".join(info), color=discord.Color.orange())
