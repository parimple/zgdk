...
class AIModerationCommands(commands.Cog):
    ...
    async def analyze_message(
        ...
        try:
            ...
            result = await self.assistant.analyze_message(
                message.content,
                user_context,
                server_rules=self._get_server_rules(ctx.guild)
            )
            ...
            embed.add_field(
                ...
                value=self._format_threat_level(result.threat_level),
                inline=True
            )
            ...
            embed.add_field(
                ...
                value=self._format_action(result.suggested_action),
                inline=True
            )
    ...
    async def toggle_auto_mod(self, ctx: commands.Context):
        ...
        if not current:
            embed.add_field(
                name="ℹ️ Informacja",
                value="Bot będzie automatycznie analizował podejrzane wiadomości",
                inline=False
            )
    ...
    async def review_decisions(self, ctx: commands.Context, limit: int = 5):
        ...
        decisions = explainer.logger.get_recent_decisions(
            limit=limit,
            module="moderation_assistant"
        )
        ...
        try:
            if self.suggestion.suggested_action == ModerationType.WARN:
                ...
                await self.message.channel.send(
                    f"{self.user.mention} - **Ostrzeżenie**: {self.suggestion.reason}"
                )
