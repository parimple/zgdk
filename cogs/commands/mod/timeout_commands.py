...
class TimeoutCommands(commands.Cog):
    ...
    @commands.hybrid_command(name="timeout", description="Nadaje użytkownikowi timeout na określony czas.")
    @is_mod_or_admin()
    @discord.app_commands.describe(
        user="Użytkownik do wyciszenia", duration="Czas trwania (np. 1h, 30m, 1d)", reason="Powód timeout'u"
    )
    async def timeout(self, ctx: commands.Context, user: discord.Member, duration: str, *, reason: str = "Brak powodu"):
        ...
        try:
            ...
            embed = discord.Embed(
                title="⏰ Timeout", description=f"{user.mention} otrzymał timeout", color=discord.Color.orange()
            )
    ...
    @commands.hybrid_command(name="untimeout", description="Usuwa timeout użytkownika.")
    @is_mod_or_admin()
    @discord.app_commands.describe(user="Użytkownik do odwyciszenia", reason="Powód usunięcia timeout'u")
    async def untimeout(self, ctx: commands.Context, user: discord.Member, *, reason: str = "Brak powodu"):
    ...
    async def mute_history(self, ctx: commands.Context, user: discord.Member, limit: int = 10):
        ...
        try:
            async with self.bot.get_db() as session:
                ...
                history = await moderation_repo.get_user_mute_history(user.id, limit)
    ...
    @commands.command(name="mutenick", description="Usuwa niewłaściwy nick użytkownika i nadaje karę.")
    @is_mod_or_admin()
    async def mutenick_prefix(self, ctx: commands.Context, user: discord.Member):
        ...
        try:
            logger.info(f"mutenick command started for user {user.id} ({user.display_name}) by {ctx.author.id}")
            ...
            logger.info(f"User {user.id} original nick: '{original_nick}', target nick: '{default_nick}'")
            ...
            if updated_user:
                ...
                if current_nick != default_nick:
                    ...
                    try:
                        ...
                        logger.info(f"Successfully enforced nick '{default_nick}' for user {user.id}")
                    except Exception as e:
