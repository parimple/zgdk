...
class ClearCommands(commands.Cog):
    ...
    @commands.hybrid_command(name="clear", description="Usuwa wiadomości użytkownika z ostatnich X godzin.")
    @is_mod_or_admin()
    ...
    async def clear_messages(self, ctx: commands.Context, user: discord.Member, hours: Optional[int] = 1):
    ...
    async def clear_all_channels(self, ctx: commands.Context, user: discord.Member, hours: Optional[int] = 1):
    ...
    async def clear_images(self, ctx: commands.Context, hours: Optional[int] = 1):
    ...
    async def modsync(self, ctx: commands.Context):
        ...
        try:
            ...
            await self.message_sender.send_success(ctx, "Komendy moderacyjne zostały zsynchronizowane!")
            logger.info("Moderation commands synced successfully")
        except Exception as e:
            logger.error(f"Error syncing moderation commands: {e}")
            await self.message_sender.send_error(ctx, f"Błąd podczas synchronizacji komend: {str(e)}")
