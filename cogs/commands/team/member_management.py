...
class MemberManagementCommands:
    ...
    async def team_invite(self, ctx, member: discord.Member):
        ...
        if not team_role:
            await self._send_premium_embed(
                ctx, description="❌ Nie posiadasz teamu!", color=0xFF0000
            )
            return
        ...
        async with self.bot.get_db() as session:
            ...
            if highest_premium:
                premium_config = next(
                    (r for r in self.bot.config["premium_roles"] if r["name"] == highest_premium.name),
                    None
                )
                if premium_config:
            ...
            if str(reaction.emoji) == "❌":
                ...
                await ctx.author.send(
                    f"❌ {member.mention} odrzucił zaproszenie do teamu."
                )
                return
            ...
            try:
                ...
                await ctx.author.send(
                    f"✅ {member.mention} dołączył do teamu **{team_role.name[2:]}**!"
                )
                
                # Grant access to team channel if exists
                team_channel = discord.utils.get(
                    ctx.guild.channels,
                    name=f"💬・{team_role.name[2:].lower().replace(' ', '-')}"
                )
                if team_channel:
                ...
                await member.send(
                    f"❌ Wystąpił błąd podczas dodawania do teamu: {str(e)}"
                )
        ...
        async with self.bot.get_db() as session:
            ...
            try:
                ...
                await self.message_sender.send_success(
                    ctx, f"{member.mention} został wyrzucony z teamu!"
                )
                ...
                await self.message_sender.send_error(
                    ctx, f"Wystąpił błąd podczas wyrzucania z teamu: {str(e)}"
                )
    ...
    async def team_leave(self, ctx):
        ...
        if not team_role:
            await self._send_premium_embed(
                ctx, description="❌ Nie należysz do żadnego teamu!", color=0xFF0000
            )
            return
        ...
        async with self.bot.get_db() as session:
            ...
            try:
                ...
                await self.message_sender.send_success(
                    ctx, f"Opuściłeś team **{team_role.name[2:]}**!"
                )
                ...
                if db_role and db_role.name.isdigit():
                    ...
                    if owner:
                        try:
                            await owner.send(
                                f"ℹ️ {ctx.author.mention} opuścił team **{team_role.name[2:]}**."
                            )
                        except:
                            pass
                ...
                await self.message_sender.send_error(
                    ctx, f"Wystąpił błąd podczas opuszczania teamu: {str(e)}"
                )
    ...
    async def team_transfer(self, ctx, member: discord.Member):
        ...
        async with self.bot.get_db() as session:
            ...
            try:
                ...
                try:
                    await member.send(
                        f"✅ Otrzymałeś własność teamu **{team_role.name[2:]}** od {ctx.author.mention}!"
                    )
                except:
                    pass
                ...
                await self.message_sender.send_error(
                    ctx, f"Wystąpił błąd podczas przekazywania teamu: {str(e)}"
                )