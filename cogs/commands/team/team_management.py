...
from cogs.commands.premium.utils import emoji_validator, emoji_to_icon
...
class TeamManagementCommands:
    ...
    async def team_create(
        ...
        async with self.bot.get_db() as session:
            ...
            if existing_role:
                await self._send_premium_embed(
                    ctx, description="❌ Team o takiej nazwie już istnieje!", color=0xFF0000
                )
                return
        ...
        if emoji:
            ...
            if not has_emoji_permission:
                ...
                if not is_valid:
                    await self._send_premium_embed(
                        ctx, description=f"❌ {error_msg}", color=0xFF0000
                    )
                else:
                    try:
    ...
    async def team_delete(self, ctx):
        ...
        async with self.bot.get_db() as session:
            ...
            try:
                ...
                await self._send_premium_embed(
                    ctx, description="❌ Czas na potwierdzenie minął.", color=0xFF0000
                )
                return
            ...
            if str(reaction.emoji) == "❌":
                await self._send_premium_embed(
                    ctx, description="❌ Usuwanie teamu anulowane.", color=0xFF0000
                )
                return
            ...
            if team_info["channel"]:
                try:
                    await team_info["channel"].delete(
            ...
            try:
                ...
                await self.message_sender.send_success(
                    ctx, "Team został pomyślnie usunięty!"
                )
            except Exception as e:
                logger.error(f"Error deleting team role: {str(e)}")
                await self.message_sender.send_error(
                    ctx, f"Wystąpił błąd podczas usuwania teamu: {str(e)}"
                )