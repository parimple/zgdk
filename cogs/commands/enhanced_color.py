...
class EnhancedColorCommands(commands.Cog):
    ...
    @commands.hybrid_command(
        name="kolor_ai",
        description="Ustaw swój kolor z pomocą AI (Premium)",
        aliases=["color_ai", "colour_ai"]
    )
    @commands.guild_only()
        ...
        try:
            ...
            if enhanced_color.confidence < 1.0:
                preview_embed.add_field(
                    ...
                    value=enhanced_color.interpretation,
                    inline=False
                )
            
            if enhanced_color.closest_named_color:
                preview_embed.add_field(
                    name="Najbliższy Nazwany Kolor",
                    value=enhanced_color.closest_named_color.title(),
                    inline=True
                )
            ...
            color_role = await self._create_or_update_color_role(
                ctx.author,
                enhanced_color.discord_color
            )
            ...
            if color_role:
                ...
                if enhanced_color.confidence < 0.9:
                    success_embed.set_footer(
                        text=f"Pewność AI: {enhanced_color.confidence:.0%}"
                    )
    ...
    async def _create_or_update_color_role(
        self,
        ...
        try:
            if color_role:
                ...
                await color_role.edit(
                    ...
                    reason=f"Color update requested by {member}"
                )
            else:
                # Create new role
                ...
                color_role = await guild.create_role(
                    name=role_name,
                    color=discord.Color(color_int),
                    reason=f"Color role for {member}"
                )
    ...
    @commands.hybrid_command(
        name="test_koloru",
        description="Testuj parsowanie kolorów AI bez aplikowania"
    )
    @commands.guild_only()
    async def color_test(self, ctx: commands.Context, *, color: str):
        ...
        try:
            # Try both parsers
            embed = discord.Embed(
                title="🎨 Test Parsowania Koloru",
                description=f"Wejście: `{color}`",
