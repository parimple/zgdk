...
class UserInfoCog(commands.Cog):
    ...
    async def _create_profile_embed(
        self, member: discord.Member, profile_data: dict, active_mutes: list, is_voice_muted: bool
    ) -> discord.Embed:
        ...
        # Wallet balance
        embed.add_field(
            name="💰 Portfel", value=f"{profile_data['db_member'].wallet_balance}{CURRENCY_UNIT}", inline=True
        )
    ...
    @commands.hybrid_command(name="profile", aliases=["p"], description="Wyświetla profil użytkownika.")
    ...
    async def bypass(
        ...
        async with self.bot.get_db() as session:
            if action == "add":
                ...
                updated_member = await MemberQueries.add_bypass_time(session, member.id, hours)
...
class SellRoleButton(discord.ui.Button):
            ...
            for role_data in roles_data:
                if role_data["expiration_date"]:
                    refund = await calculate_refund(
                        role_data["role_name"], role_data["expiration_date"], self.bot.config
                    )
    ...
    class ConfirmSaleView(discord.ui.View):
        ...
        def __init__(self, user, refund_info, bot):
            ...
            self.select = discord.ui.Select(placeholder="Wybierz rolę do sprzedania...", options=options, row=0)
        ...
        async def select_callback(self, interaction: discord.Interaction):
            ...
            if not selected_info:
                await interaction.response.send_message("❌ Nie znaleziono wybranej roli.", ephemeral=True)
                return
            ...
            embed.add_field(name="Otrzymasz", value=f"{selected_info['refund']}{CURRENCY_UNIT}", inline=True)
            embed.add_field(name="Pozostały czas", value=selected_info["time_left"], inline=True)
            ...
            async def confirm(i: discord.Interaction):
                ...
                async with self.bot.get_db() as session:
                    try:
                        ...
                        await RoleQueries.delete_member_role(session, i.user.id, selected_role_id)

                        # Add refund to wallet
                        await MemberQueries.add_to_wallet_balance(session, i.user.id, selected_info["refund"])

                        # Remove premium permissions
                        await remove_premium_role_mod_permissions(session, self.bot, i.user.id)
                        ...
                        success_embed.add_field(
                            name="Otrzymano", value=f"{selected_info['refund']}{CURRENCY_UNIT}", inline=True
                        )
                        ...
                        await i.response.send_message("❌ Wystąpił błąd podczas sprzedaży roli.", ephemeral=True)
            ...
            confirm_btn = discord.ui.Button(label="✅ Potwierdź", style=discord.ButtonStyle.success)
            confirm_btn.callback = confirm

            cancel_btn = discord.ui.Button(label="❌ Anuluj", style=discord.ButtonStyle.danger)
