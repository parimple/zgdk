        ...
        async with self.bot.get_db() as session:
            try:
                ...
                await RoleQueries.delete_member_role(
                    session, interaction.user.id, self.selected_role_id
                )
                
                # Add refund to wallet
                await MemberQueries.add_to_wallet_balance(
                    session, interaction.user.id, self.selected_info['refund']
                )
                
                # Remove premium permissions
                await remove_premium_role_mod_permissions(
                    session, self.bot, interaction.user.id
                )
...
class RoleSelectDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = create_sale_confirmation_embed(
            selected_info['role_data']['role_name'],
            selected_info['refund'],
            selected_info['time_left']
        )
