...
class HelpInfoCog(commands.Cog):
    ...
    @commands.hybrid_command(
        name="pomoc", aliases=["help"], description="Wyświetla listę komend."
    )
    @is_admin()
    ...
    async def help_command(self, ctx: commands.Context):
        ...
        embed.set_footer(
            text=f"Prefix: {self.bot.command_prefix} | Wspierane są również komendy slash (/)"
        )

        await ctx.send(embed=embed)

    @commands.hybrid_command(
        name="games", aliases=["gry"], description="Pokazuje aktywne gry na serwerze."
    )
    async def games(self, ctx: commands.Context):
        ...
        if not games_counter:
            no_games_embed = discord.Embed(
                title="🎮 Aktywne Gry",
                description="Obecnie nikt nie gra w żadne gry.",
