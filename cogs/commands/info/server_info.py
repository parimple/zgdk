...
class ServerInfoCog(commands.Cog):
    ...
    @commands.hybrid_command(
        name="serverinfo", aliases=["si"], description="Pokazuje informacje o serwerze."
    )
    async def guild_info(self, ctx: commands.Context):
        ...
        embed.add_field(
            ...
            value=verification_levels.get(guild.verification_level, "Nieznany"),
            inline=True
        )
        ...
        embed.add_field(
            name="Filtr treści",
            value=content_filters.get(guild.explicit_content_filter, "Nieznany"),
            inline=True
        )
        
        embed.set_footer(
            text=f"Żądane przez {ctx.author.display_name}",
    ...
    @commands.hybrid_command(
        name="roles", aliases=["allroles"], description="Wyświetla wszystkie role na serwerze."
    )
    async def all_roles(self, ctx: commands.Context):
        ...
        for i in range(0, len(roles), 20):
            ...
            embed = discord.Embed(
                title=f"Role na serwerze {guild.name}",
                color=discord.Color.blue(),
                timestamp=datetime.now(timezone.utc)
            )
            ...
            embed.set_footer(
                text=f"Strona {i // 20 + 1}/{(len(roles) - 1) // 20 + 1} | "
                     f"Łącznie ról: {len(roles)}"
            )
        ...
        if not pages:
            embed = discord.Embed(
                title="Role na serwerze",
                description="Brak ról do wyświetlenia.",
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user != self.author:
            await interaction.response.send_message(
                "Tylko osoba, która użyła komendy może przewijać strony.",
                ephemeral=True
            )
            return False
        return True
