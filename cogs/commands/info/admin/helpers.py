    ...
    for role in guild.roles:
        if role.name.startswith(team_symbol):
            ...
            if role.permissions.manage_roles:
                ...
                members_with_manage = [
                    m for m in role.members if m.guild_permissions.manage_roles
                ]
                ...
                if member in role.members:
                    ...
                    is_owner = any(
                        m == member and m.guild_permissions.manage_roles
...
async def get_member_voice_permissions_info(session, guild: discord.Guild, member: discord.Member):
    ...
    voice_permissions = await ChannelPermissionQueries.get_member_mod_permissions(
        session, member.id
    )
