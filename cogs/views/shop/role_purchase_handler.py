...
from .embed_helpers import (
    create_upgrade_embed, create_downgrade_embed, create_extension_embed,
    create_purchase_embed, create_error_embed, create_cancel_embed
)
...
class RolePurchaseHandler:
    ...
    async def handle_buy_role(
        ...
        role_config = next(
            (r for r in premium_roles if r["name"] == role_name), None
        )
        if not role_config:
            await interaction.response.send_message(
                "Nie znaleziono konfiguracji roli.", ephemeral=True
            )
            return
        ...
        if balance < price:
            await interaction.response.send_message(
                f"Nie masz wystarczająco środków. Potrzebujesz {price} zł.",
                ephemeral=True
            )
            return
        ...
        if current_highest_role:
            ...
            is_upgrade = RoleValidation.is_role_upgrade(
                current_highest_role, role_name, premium_roles
            )
            is_downgrade = RoleValidation.is_role_downgrade(
                current_highest_role, role_name, premium_roles
            )
            
            if is_upgrade:
                await self._handle_role_upgrade(
                    interaction, ctx, member, current_highest_role, 
                    role_name, price, days_to_add
                )
            elif is_downgrade:
                # For downgrades, automatically apply partial extension
                await self._handle_partial_extension(
                    interaction, ctx, member, current_highest_role,
                    role_name, price
                )
            else:
                # Same role - extend
                await self._handle_role_extension(
                    interaction, ctx, member, role_name, price, days_to_add
                )
        else:
            # First time purchase
            await self._handle_first_purchase(
                interaction, ctx, member, role_name, price, days_to_add
            )
    ...
    async def _handle_role_upgrade(
        ...
        choice_view = LowerRoleChoiceView(
            self.bot, member, current_role, new_role, 
            price, days_to_add, is_upgrade=True
        )
        ...
        await interaction.response.send_message(
            embed=embed, view=choice_view, ephemeral=True
        )
        
        await choice_view.wait()
        
        if choice_view.value == "buy_higher":
            await self._process_role_upgrade(
                ctx, member, current_role, new_role, 
                price, days_to_add, refund_amount
            )
    ...
    async def _handle_role_downgrade(
        ...
        choice_view = LowerRoleChoiceView(
            self.bot, member, current_role, new_role,
            price, days_to_add, is_upgrade=False
        )
    ...
    async def _handle_partial_extension(
        ...
        if current_role_price == 0:
            await interaction.response.send_message(
                "Nie można obliczyć częściowego przedłużenia.", ephemeral=True
            )
            return
        ...
        async with self.bot.get_db() as session:
            ...
            if success:
                ...
                member_color = member.color if member.color.value != 0 else discord.Color.blurple()
                description = f"✅ Ranga `{current_role}` przedłużona o `{partial_days} dni`\n(za zakup {new_role} - {price} G)"
                if had_mute_roles:
    ...
    async def _process_role_purchase(
        ...
        try:
            ...
            # Add/extend role
            if is_extension:
                await RoleQueries.extend_member_role(
                    session, member.id, discord_role.id, days_to_add
                )
            else:
                expiration_date = datetime.now(timezone.utc) + timedelta(days=days_to_add)
                await RoleQueries.add_member_role(
                    session, member.id, discord_role.id, expiration_date
                )
                # Add Discord role
            ...
            try:
                ...
                if old_discord_role:
                    ...
                    await RoleQueries.delete_member_role(
                        session, member.id, old_discord_role.id
                    )
                ...
                success = await self._process_role_purchase(
                    session, member, new_role, actual_cost, days_to_add, False
                )
