...
class RoleShopView(discord.ui.View):
    ...
    def __init__(
        ...
        if page == 1:
            next_button = discord.ui.Button(label="Pokaż ceny roczne ➡️", style=discord.ButtonStyle.secondary)
            next_button.callback = self.next_page
            self.add_item(next_button)
        else:
            previous_button = discord.ui.Button(label="⬅️ Pokaż ceny miesięczne", style=discord.ButtonStyle.secondary)
            previous_button.callback = self.previous_page
            self.add_item(previous_button)

        # Add action buttons
        description_button = discord.ui.Button(label="📋 Opis ról", style=discord.ButtonStyle.secondary)
        description_button.callback = self.show_role_description
        self.add_item(description_button)

        my_id_button = discord.ui.Button(label="🆔 Moje ID", style=discord.ButtonStyle.secondary)
    ...
    def create_button_callback(self, role_name):
        ...
        async def button_callback(interaction: discord.Interaction):
            ...
            await self.purchase_handler.handle_buy_role(
                interaction, self.ctx, self.member, role_name, self.page, self.balance, self.premium_roles
            )
    ...
    async def handle_buy_role(
        ...
        await self.purchase_handler.handle_buy_role(
            interaction, self.ctx, member, role_name, self.page, self.balance, self.premium_roles
        )
    ...
    async def show_my_id(self, interaction: discord.Interaction):
        ...
        user_id = interaction.user.id if interaction.user.id != self.viewer.id else self.viewer.id
        await interaction.response.send_message(f"Twoje ID Discord: `{user_id}`", ephemeral=True)
