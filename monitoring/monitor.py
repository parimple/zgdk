...
class ZGDKMonitor:
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        if not config_file.exists():
            ...
            default_config = {
                "monitoring": {"interval_seconds": 300, "timeout_seconds": 30, "retry_attempts": 3},
                "github": {"owner": "your-github-username", "repo": "zgdk", "workflows": ["ci", "docker-build"]},
                "docker": {"containers": ["zgdk_app", "zgdk_db", "zgdk_redis"], "compose_file": "docker-compose.yml"},
                "argocd": {"enabled": False, "server": "argocd.example.com", "app_name": "zgdk"},
                "notifications": {
                    "webhook_url": "",
                    "email": {"enabled": False, "smtp_server": "", "smtp_port": 587, "from_email": "", "to_emails": []},
                },
                "status_page": {"output_path": "monitoring/status.html", "update_interval": 60},
            }
    ...
    def check_docker_containers(self) -> List[HealthCheck]:
        ...
        for container in self.config["docker"]["containers"]:
            try:
                ...
                checks.append(
                    HealthCheck(service=f"docker_{container}", status=status, message=message, timestamp=datetime.now())
                )
...
<body>
    ...
    async def run_health_checks(self):
        ...
        # Log results
        failed_checks = [
            check for check in self.health_checks if check.status not in [ServiceStatus.HEALTHY, ServiceStatus.UNKNOWN]
        ]
