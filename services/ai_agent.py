...
class AIAgentService:
    ...
    async def setup(self):
        ...
        self.redis_client = await redis.from_url(f"redis://{os.getenv('REDIS_HOST', 'localhost')}:6379")

        # Setup AI agent
        genai.configure(api_key=os.getenv("GEMINI_API_KEY"))
        self.agent = Agent("gemini-1.5-flash", system_prompt="Jesteś pomocnym asystentem Discord bota zaGadka.")
    ...
    async def process_task(self, request: AIRequest) -> AIResponse:
        ...
        if cached:
            ...
            return AIResponse(task_id=request.task_id, result=cached.decode(), metadata={"cached": True})
        ...
        try:
            ...
            # Cache result
            await self.redis_client.setex(cache_key, 3600, result.data)  # 1 hour TTL

            return AIResponse(
                task_id=request.task_id, result=result.data, metadata={"cached": False, "model": "gemini-1.5-flash"}
            )
    ...
    async def get_queue_status(self) -> Dict[str, Any]:
        ...
        return {"queue_length": queue_length, "processing": len(self.processing), "healthy": True}
