...
class ModerationAssistant:
    ...
    def __init__(self, use_ai: bool = True):
        ...
        if use_ai:
            ...
            if gemini_key:
                ...
                self.agent = Agent("openai:gpt-4", system_prompt=system_prompt)  # Use GPT-4 for better moderation
            else:
                self.use_ai = False
    ...
    async def analyze_message(
        self, message: str, user_context: UserContext, server_rules: Optional[List[str]] = None
        ...
        if self.use_ai:
            ai_result = await self._analyze_with_ai(message, user_context, server_rules)
    ...
    def _analyze_patterns(self, message: str, user_context: UserContext) -> Optional[ModerationSuggestion]:
    ...
    async def _analyze_with_ai(
        ...
        try:
            ...
            # Get AI analysis
            result = await self.agent.run(f"Analyze this Discord message for violations:\n{context}")
    ...
    def _combine_analyses(
        self, pattern_result: ModerationSuggestion, ai_result: ModerationSuggestion
    ) -> ModerationSuggestion:
        """Combine pattern and AI analyses."""
        # Use the higher threat level
        threat_level = max(
            pattern_result.threat_level, ai_result.threat_level, key=lambda x: list(ThreatLevel).index(x)
        )

        # Combine violations
        all_violations = list(set(pattern_result.violations + ai_result.violations))
        ...
        if action_severity.get(pattern_result.suggested_action, 0) > action_severity.get(ai_result.suggested_action, 0):
    ...
    async def analyze_user_behavior(
        self, user_context: UserContext, time_window_hours: int = 24
    ) -> ModerationSuggestion:
