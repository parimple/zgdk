...
class ActivityRepository(BaseRepository):
    ...
    async def get_member_activity(
        ...
        try:
            ...
            self._log_operation(
                "get_member_activity", member_id=member_id, activity_type=activity_type, count=len(activities)
            )
    ...
    async def add_activity(
        ...
        try:
            ...
            activity = Activity(member_id=member_id, points=points, activity_type=activity_type, date=date)

            self.session.add(activity)
            await self.session.flush()

            self._log_operation("add_activity", member_id=member_id, points=points, activity_type=activity_type)
    ...
    async def get_member_total_points(
        ...
        try:
            ...
            self._log_operation(
                "get_member_total_points", member_id=member_id, activity_type=activity_type, total_points=total_points
            )
    ...
    async def get_leaderboard(
        ...
        try:
            ...
            query = query.group_by(Member.id).order_by(func.sum(Activity.points).desc()).limit(limit)
            ...
            self._log_operation("get_leaderboard", activity_type=activity_type, limit=limit, count=len(leaderboard))
    ...
    async def get_total_points(
        ...
        try:
            ...
            self._log_operation("get_total_points", activity_type=activity_type, total_points=total_points)
    ...
    async def get_active_members_count(
        ...
        try:
            ...
            self._log_operation("get_active_members_count", count=count)
    ...
    async def get_member_activity_summary(self, member_id: int, days: int = 30) -> dict[str, any]:
        ...
        try:
            ...
            query = (
                ...
                .where(Activity.member_id == member_id, Activity.date >= cutoff_date)
            ...
            for row in result:
                ...
                breakdown[row.activity_type] = {"points": points, "sessions": sessions}
                total_points += points
            ...
            self._log_operation(
                "get_member_activity_summary", member_id=member_id, days=days, total_points=total_points
            )
            ...
            return {"total_points": 0, "total_sessions": 0, "breakdown": {}, "period_days": days}
