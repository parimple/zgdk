...
class NotificationRepository(BaseRepository):
    ...
    async def add_or_update_notification_log(
        ...
        """
        notification_log = await self.session.get(NotificationLog, (member_id, notification_tag))
        ...
        logger.info(f"Updated notification log: member={member_id}, tag={notification_tag}")
    ...
    async def increment_notification_count(
        ...
        should_opt_out = notification_log.notification_count >= self.MAX_NOTIFICATION_COUNT
        if should_opt_out:
            notification_log.opted_out = True
            logger.info(f"Auto opt-out for member={member_id}, tag={notification_tag}")
    ...
    async def get_notification_log(self, member_id: int, notification_tag: str) -> Optional[NotificationLog]:
    ...
    async def get_service_users(self, service: str, guild_id: Optional[int] = None) -> List[int]:
    ...
    async def process_service_usage(
        ...
        log = await self.get_service_notification_log(service, guild_id, user_id)

        # If no log exists, service can be used
        if not log:
            if not dry_run:
                log = await self.add_or_update_notification_log(user_id, service)
            return True, log
