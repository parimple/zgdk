...
class AutoKickRepository(BaseRepository):
    ...
    async def get_owner_autokicks(self, owner_id: int) -> List[AutoKick]:
        ...
        """
        result = await self.session.execute(select(AutoKick).where(AutoKick.owner_id == owner_id))
        return list(result.scalars().all())
    ...
    async def get_target_autokicks(self, target_id: int) -> List[AutoKick]:
        ...
        """
        result = await self.session.execute(select(AutoKick).where(AutoKick.target_id == target_id))
        return list(result.scalars().all())
    ...
    async def is_autokicked(self, owner_id: int, target_id: int) -> bool:
        ...
        result = await self.session.scalar(
            select(AutoKick.id).where((AutoKick.owner_id == owner_id) & (AutoKick.target_id == target_id)).limit(1)
        )
