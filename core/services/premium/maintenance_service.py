        ...
        try:
            ...
            for premium_role in expired:
                ...
                if not member:
                    ...
                    await self.premium_repository.remove_premium_role(
                        premium_role.member_id, 
                        premium_role.role_name
                    )
                    continue
                ...
                if role:
                    try:
                        ...
                        logger.info(
                            f"Removed expired premium role {premium_role.role_name} "
                            f"from {member}"
                        )
                        ...
                        logger.error(
                            f"Error removing role {premium_role.role_name} "
                            f"from {member}: {e}"
                        )
        ...
        try:
            ...
            for premium_role in all_premium:
                ...
                if not member:
                    ...
                    cleaned += 1
                    logger.info(
                        f"Cleaned up premium entry for absent member "
                        f"{premium_role.member_id}"
                    )
        ...
        try:
            ...
            for premium_role in all_premium:
                ...
                if member:
                    ...
                    days_until_expiry = (
                        premium_role.expires_at - datetime.now(timezone.utc)
                    ).days
