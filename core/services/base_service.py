...
from core.exceptions import (
    ...
    ValidationException,
    PermissionError as PermissionException,
...
class BaseService(IService):
            ...
            try:
                ...
                if not isinstance(e, BotError):
                    raise TransactionException(
                        f"Transaction failed in {operation.__name__}",
                        original_error=e
                    ) from e
                raise
    ...
    async def validate_input(self, data: Dict[str, Any], validators: Dict[str, Callable]) -> Dict[str, Any]:
        ...
        for field, validator in validators.items():
            ...
            try:
                validated[field] = await validator(data[field]) if asyncio.iscoroutinefunction(validator) else validator(data[field])
            except Exception as e:
