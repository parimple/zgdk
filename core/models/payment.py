...
class PremiumPurchaseRequest(BaseModel):
    ...
    def validate_tier_amount(cls, v: Decimal, values: dict) -> Decimal:
        ...
        if tier and currency == Currency.PLN:
            ...
            if expected and abs(v - expected) > Decimal("0.01"):
                raise ValueError(f"Amount {v} doesn't match {tier.value} price {expected} PLN")
        return v
    ...
    def validate_team_size(cls, v: list[DiscordID], values: dict) -> list[DiscordID]:
        ...
        tier_limits = {PremiumTier.ZG50: 0, PremiumTier.ZG100: 10, PremiumTier.ZG500: 20, PremiumTier.ZG1000: 30}

        tier = values.get("tier")
        if tier:
            limit = tier_limits.get(tier, 0)
            if len(v) > limit:
                raise ValueError(f"Team size {len(v)} exceeds limit {limit} for {tier.value}")
        return v
