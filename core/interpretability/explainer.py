...
class ActionExplainer:
    ...
    def explain_decision(self, decision: Decision, level: ExplanationLevel = ExplanationLevel.SIMPLE) -> str:
    ...
    def _detailed_explanation(self, decision: Decision) -> str:
        ...
        if decision.decision_type == DecisionType.PERMISSION_CHECK:
            conditions = "\n".join(
                f"• {cond['permission']}: {'✅' if cond['has'] else '❌'}" for cond in decision.checked_conditions
            )
    ...
    def create_explanation_embed(
        ...
        if level in [ExplanationLevel.DETAILED, ExplanationLevel.TECHNICAL]:
            embed.add_field(name="Typ decyzji", value=decision.decision_type.value, inline=True)
            embed.add_field(name="Pewność", value=f"{decision.confidence * 100:.0f}%", inline=True)
