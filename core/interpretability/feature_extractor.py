...
class FeatureExtractor:
    ...
    def _initialize_features(self):
        ...
        self.add_feature(
            ...
            "Aktywuje się gdy komenda wymaga uprawnień administratora"
        )
        self.add_feature(
            "perm_premium_required",
            "Wymaga Premium",
            "Aktywuje się gdy funkcja wymaga rangi premium"
        )
        self.add_feature(
            "perm_team_leader",
            "Wymaga Lidera Drużyny",
            "Aktywuje się gdy akcja wymaga bycia liderem drużyny"
        )
        ...
        self.add_feature(
            "mod_spam_detected",
            "Wykryto Spam",
            "Aktywuje się przy wykryciu spamu lub flood"
        )
        self.add_feature(
            "mod_repeat_offender",
            "Recydywista",
            "Aktywuje się gdy użytkownik ma historię naruszeń"
        )
        self.add_feature(
            "mod_severity_high",
            "Wysokie Zagrożenie",
            "Aktywuje się przy poważnych naruszeniach"
        )
        ...
        self.add_feature(
            ...
            "Brak Środków",
            "Aktywuje się gdy użytkownik nie ma wystarczających środków"
        )
        self.add_feature(
            "econ_role_upgrade",
            "Upgrade Rangi",
            "Aktywuje się przy próbie ulepszenia rangi"
        )
        self.add_feature(
            "econ_refund_calculated",
            "Obliczono Zwrot",
            "Aktywuje się gdy system oblicza zwrot za rangę"
        )
        ...
        self.add_feature(
            "team_full",
            "Drużyna Pełna",
            "Aktywuje się gdy drużyna osiągnęła limit członków"
        )
        self.add_feature(
            "team_hierarchy",
            "Hierarchia Drużyny",
            "Aktywuje się przy sprawdzaniu hierarchii w drużynie"
        )
        ...
        self.add_feature(
            "cooldown_active",
            "Aktywny Cooldown",
            "Aktywuje się gdy użytkownik jest na cooldownie"
        )
        self.add_feature(
            "cooldown_bump_service",
            "Cooldown Bump",
            "Aktywuje się dla cooldownów serwisów bump"
        )
        ...
        self.add_feature(
            "ai_natural_language",
            "Język Naturalny",
            "Aktywuje się przy przetwarzaniu naturalnego języka"
        )
        self.add_feature(
            "ai_low_confidence",
            "Niska Pewność AI",
            "Aktywuje się gdy AI ma niską pewność decyzji"
        )
    ...
    def get_feature_map(self) -> Dict[str, Dict]:
        ...
        for feature_id, feature in self.features.items():
            ...
            feature_map[feature_id] = {
                ...
                "related_features": [
                    {"id": rel_id, "correlation": corr}
                    for rel_id, corr in related[:5]
                ],
    ...
    def export_feature_analysis(self, output_path: str = "feature_analysis.json"):
        ...
        analysis = {
            ...
            "feature_correlations": {
                feature_id: self.find_related_features(feature_id)
