...
class CommandTracer:
    ...
    async def trace_command(self, ctx: commands.Context):
        ...
        trace_ctx = TraceContext(
            command_name=ctx.command.name, user_id=str(ctx.author.id), guild_id=str(ctx.guild.id) if ctx.guild else None
        )
    ...
    def get_command_performance(self, command_name: str) -> Dict[str, Any]:
        ...
        command_traces = [trace for trace in self.completed_traces if trace.command_name == command_name]
    ...
    async def trace_async_operation(
        self, operation_name: str, func, *args, trace_ctx: Optional[TraceContext] = None, **kwargs
    ):
