...
class AutomatedBotTester:
    ...
    def __init__(self, bot_token: str, test_channel_id: int):
        ...
        self.headers = {"Authorization": f"Bot {bot_token}", "Content-Type": "application/json"}
        self.session = None
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            async with self.session.get(url, headers=self.headers, params=params) as response:
                if response.status == 200:
                    ...
                    bot_messages = [msg for msg in messages if msg.get("author", {}).get("bot", False)]
                    if bot_messages:
    ...
    async def test_command(self, command: str, expected_patterns: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if "error" in sent_message:
            return {"command": command, "success": False, "error": sent_message["error"], "responses": []}
...
class TestSuite:
    ...
    async def test_profile_command(self):
        ...
        result = await self.tester.test_command(
            "!profile", expected_patterns=["Profil użytkownika", "Portfel", "Zaproszenia"]
        )
        self.results.append(("Profile Command", result))

    async def test_shop_command(self):
        """Test shop command."""
        result = await self.tester.test_command("!shop", expected_patterns=["Sklep", "Premium", "zG"])
        self.results.append(("Shop Command", result))

    async def test_help_command(self):
        """Test help command."""
        result = await self.tester.test_command("!help", expected_patterns=["Komendy", "pomoc", "help"])
    ...
    async def run_all_tests(self):
        ...
        with open(filename, "w") as f:
            json.dump([{"test": name, "result": result} for name, result in self.results], f, indent=2)
