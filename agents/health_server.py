...
class HealthServer:
    ...
    async def health_check(self, request):
        ...
        if self.healthy:
            return web.json_response(
                {"status": "healthy", "agent": self.agent_type, "uptime": str(datetime.now() - self.startup_time)}
            )
    ...
    async def readiness_check(self, request):
        ...
        if self.ready:
            return web.json_response({"status": "ready", "agent": self.agent_type})
...
async def main():
    ...
    try:
        ...
        redis_client = await redis.from_url(f"redis://{os.getenv('REDIS_HOST', 'localhost')}:6379")
