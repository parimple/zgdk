...
from utils.errors import ZGDKError, ResourceNotFoundError, PermissionError
...
class VoiceService(BaseService):
    ...
    async def get_permission_level(self, ctx, channel: Optional[VoiceChannel] = None) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            permission_level = await self.voice_manager.get_permission_level(voice_channel, ctx.author)
    ...
    async def modify_permission(
        ...
        try:
            ...
            can_modify = await self.voice_manager.can_modify_permissions(voice_channel, ctx.author, target)
            if not can_modify:
            ...
            if success:
                ...
                value_desc = "enabled" if value == "+" else "disabled" if value == "-" else "toggled"
                target_desc = target.name if isinstance(target, discord.Role) else f"{target.display_name}"
                
                return True, f"{permission_name.replace('_', ' ').title()} {value_desc} for {target_desc}"
            else:
    ...
    async def reset_user_permissions(self, ctx, target: Member) -> Tuple[bool, str]:
        ...
        try:
            ...
            success = await self.voice_manager.reset_user_permissions(voice_channel, ctx.author, target)
