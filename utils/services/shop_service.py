...
class ShopService(BaseService):
    ...
    async def assign_payment(self, payment_id: int, user: discord.User) -> Tuple[bool, str]:
        ...
        if success:
            ...
            try:
                await user.send("Proszę pamiętać o podawaniu swojego ID podczas dokonywania wpłat w przyszłości. Twoje ID to:")
    ...
    async def get_recent_payments(self, limit: int = 10, offset: int = 0) -> Tuple[bool, str, Optional[List[Any]]]:
        """Get recent payments.
    ...
    async def set_role_expiry(self, member: discord.Member, hours: int, role_id: Optional[int] = None) -> Tuple[bool, str, Optional[datetime]]:
