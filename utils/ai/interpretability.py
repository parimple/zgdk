...
class FeatureExtractor:
    ...
    async def extract_duration_features(text: str) -> Dict[str, Any]:
        ...
        features = {
            ...
            "has_unit_keywords": any(unit in text.lower() for unit in [
                "dni", "dzień", "godzin", "minut", "sekund", "tyg", "mies", "rok",
                "day", "hour", "minute", "second", "week", "month", "year"
            ]),
            "has_polish_numerals": any(num in text.lower() for num in [
                "jeden", "dwa", "trzy", "cztery", "pięć", "sześć", "siedem",
                "osiem", "dziewięć", "dziesięć", "dwadzieścia", "trzydzieści"
            ]),
            "has_relative_terms": any(term in text.lower() for term in [
                "pół", "połowa", "kwadrans", "półtora"
            ]),
    ...
    async def extract_color_features(text: str) -> Dict[str, Any]:
        ...
        features = {
            ...
            "has_color_name": any(color in text.lower() for color in [
                "czerwon", "zielon", "niebiesk", "żółt", "fiolet", "różow",
                "czarn", "biał", "szar", "brązow", "pomarańcz",
                "red", "green", "blue", "yellow", "purple", "pink",
                "black", "white", "gray", "brown", "orange"
            ]),
            "has_modifier": any(mod in text.lower() for mod in [
                "jasn", "ciemn", "pastel", "neon", "metalic",
...
class ModelExplainer:
    ...
    def _explain_intent(self, input_text: str, decision: Any, features: Dict[str, Any], confidence: float) -> str:
        ...
        for action, present in actions.items():
            if present:
                action_pl = {"add": "dodawanie", "remove": "usuwanie", 
                           "check": "sprawdzanie", "modify": "modyfikacja"}.get(action, action)
