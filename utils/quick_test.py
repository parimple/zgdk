...
class QuickTestContext:
    ...
    def __init__(self, bot, command_name: str, **kwargs):
        ...
        self.author = MagicMock(
            id=kwargs.get("author_id", 123456789), name=kwargs.get("author_name", "TestUser"), mention="<@123456789>"
        )
        self.guild = MagicMock(id=kwargs.get("guild_id", 987654321), name=kwargs.get("guild_name", "TestGuild"))
        self.channel = MagicMock(
            id=kwargs.get("channel_id", 111111111), name=kwargs.get("channel_name", "test-channel")
        )
    ...
    async def _capture_send(self, content=None, embed=None, **kwargs):
        ...
        self.responses.append({"type": "send", "content": content, "embed": embed, "kwargs": kwargs})
        return MagicMock()

    async def _capture_reply(self, content=None, embed=None, **kwargs):
        """Capture replies."""
        self.responses.append({"type": "reply", "content": content, "embed": embed, "kwargs": kwargs})
        return MagicMock()
...
class QuickTester:
    ...
    async def test_command(self, command_path: str, *args, **context_kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not command:
                return {"success": False, "error": f"Command {command_path} not found"}
            ...
            return {"success": True, "responses": ctx.responses, "command": command.name}
    ...
    async def test_multiple(self, tests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        tasks = []
        for test in tests:
            task = self.test_command(test["command"], *test.get("args", []), **test.get("context", {}))
            tasks.append(task)
