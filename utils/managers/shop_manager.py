...
from datasources.queries import MemberQueries, RoleQueries, HandledPaymentQueries
from utils.errors import ResourceNotFoundError, BusinessRuleViolationError
...
class ShopManager(BaseManager):
    ...
    async def get_recent_payments(self, limit: int = 10, offset: int = 0) -> List[Any]:
        ...
        """
        async with self.bot.get_db() as session:
            payments = await HandledPaymentQueries.get_last_payments(session, limit=limit, offset=offset)
            return payments
