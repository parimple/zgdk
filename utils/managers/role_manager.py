...
class RoleManager(BaseManager):
        ...
        try:
            ...
            async with self.bot.get_db() as session:
                ...
                for member_role in expired_roles:
                    ...
                    if not member:  # Should be handled by exceptions above, but as an additional safeguard
                        stats["non_existent_members"] += 1
                        stats["skipped_member_ids"].add(member_role.member_id)
                        logger.warning(
    ...
    async def send_default_notification(self, member: discord.Member, role: discord.Role):
        ...
        try:
            ...
            if is_mute_role:
                message = (
                    f"Your mute ({role.name}) has expired and has been automatically removed."
                )
            else:
    ...
    async def add_role_with_expiry(self, member_id: int, role_id: int, expiry_hours: int) -> bool:
        ...
        try:
            ...
            logger.info(f"Added role {role.name} to {member.display_name} with {expiry_hours}h expiry")
            return True
    ...
    async def get_role_info(self, member_id: int, role_id: Optional[int] = None) -> Tuple[bool, Dict[str, Any]]:
            ...
            async with self.bot.get_db() as session:
                if role_id:
                    ...
                    return True, {
                        ...
                        "has_role_on_discord": discord_role in self.bot.guild.get_member(member_id).roles if discord_role and self.bot.guild.get_member(member_id) else False
                    }
                else:
                    # Get all roles
