...
import discord
from discord import Member, PermissionOverwrite, Permissions, VoiceChannel, TextChannel, CategoryChannel

from datasources.queries import AutoKickQueries, ChannelPermissionQueries
from utils.errors import ResourceNotFoundError, PermissionError
...
class VoiceManager(BaseManager):
    ...
    async def get_permission_level(self, channel: VoiceChannel, member: Member) -> Literal["owner", "mod", "none"]:
    ...
    async def can_modify_permissions(self, channel: VoiceChannel, member: Member, target=None) -> bool:
    ...
    async def reset_user_permissions(self, channel: VoiceChannel, member: Member, target: Member) -> bool:
