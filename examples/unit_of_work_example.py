    ...
    async def unit_of_work_example(self, ctx, member: discord.Member):
        ...
        async with self.bot.get_unit_of_work() as uow:
            try:
                ...
                if not db_member:
                    db_member = await uow.members.create_member(discord_id=member.id, joined_at=member.joined_at)

                # Add activity points
                await uow.activities.add_activity(member_id=db_member.id, points=10, activity_type="bonus")

                # Update wallet balance
                await uow.members.update_wallet_balance(db_member.id, db_member.wallet_balance + 100)
                ...
                try:
                    ...
                    if db_member:
                        ...
                        activities = await uow.activities.get_member_activity(
                            member_id=db_member.id, activity_type="bonus"
                        )
                        ...
                        if total_bonus_points > 1000:
                            ...
                            await uow.activities.add_activity(
                                member_id=db_member.id, points=500, activity_type="achievement"
                            )

                            # Update wallet with bonus
                            new_balance = db_member.wallet_balance + 1000
                            await uow.members.update_wallet_balance(db_member.id, new_balance)

                            result_msg = f"🎉 {member.mention} earned special achievement! +1000 wallet bonus!"
                        else:
                            result_msg = (
                                f"📊 {member.mention} has {total_bonus_points} bonus points (need 1000 for achievement)"
                            )
                    else:
