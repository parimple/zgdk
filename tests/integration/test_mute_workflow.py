...
class TestMuteWorkflow(CommandTestCase):
    ...
    def test_txt_mute_unmute_cycle(self):
        ...
        # First mute the user
        mute_result = self.run_async(self.execute_command("mute txt", f"{TEST_USER_ID} 1h"))
        ...
        # Now unmute
        unmute_result = self.run_async(self.execute_command("unmute txt", TEST_USER_ID))
        ...
        self.assertTrue(assert_mute_response_valid(mute_response, TEST_USER_ID, "txt", is_unmute=False))
        self.assertTrue(assert_mute_response_valid(unmute_response, TEST_USER_ID, "txt", is_unmute=True))
    ...
    def test_multiple_mute_types(self):
        ...
        for mute_type in mute_types:
            result = self.run_async(self.execute_command(f"mute {mute_type}", TEST_USER_ID))
    ...
    def test_override_existing_mute(self):
        ...
        first_result = self.run_async(self.execute_command("mute img", f"{TEST_USER_ID} 1h"))
        ...
        second_result = self.run_async(self.execute_command("mute img", f"{TEST_USER_ID} 2h"))
